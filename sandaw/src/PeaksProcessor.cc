...
void SandixPeaksProcessor::Downsample(std::vector<double>& dWaveForm) {
	//The number of samples within this bigger sample
	unsigned int iWaveFormSize = dWaveForm.size();
	unsigned int iBiggerSample = ((unsigned int)(iWaveFormSize / m_pConfig->m_iMaxSamples) + 1);
	unsigned int iNumBiggerSamples = (unsigned int)(iWaveFormSize / iBiggerSample);

	unsigned int iWaveformPos = 0;
	m_iDownsampledWaveform.resize(m_pConfig->m_iMaxSamples,0.);

	for (int i = 0; i < iNumBiggerSamples; i++) {
		for (int j = 0; j < iBiggerSample; j++) {
			m_iDownsampledWaveform[i] += dWaveForm[iWaveformPos];
			++iWaveformPos;
		}
	}

	for (int k = 0; k < iWaveFormSize - iNumBiggerSamples * iBiggerSample; k++) {
		m_iDownsampledWaveform[iNumBiggerSamples] += dWaveForm[iWaveformPos];
		++iWaveformPos;
	}
}

double SandixPeaksProcessor::PercentileTime(std::vector<double>& dCWf, double dPercentile) {
	/*
	:param: dCWf: The cumulative waveform
	:param: dPercentile: The percentile for which to see when the waveform reaches
	*/

	double dTotalArea = dCWf[dCWf.size() - 1];
	int s = 0;
...
double SandixPeaksProcessor::HeightTime(std::vector<double>& dWf, double dPercentHeight) {
	double dMaxHeight = *std::max_element(dWf.begin(), dWf.end());
	int s = 0;
...
void SandixPeaksProcessor::ProcessPeaks(Hits_t& Hits, bool bSave = false, std::string sOutFile = "") {
	
	if (sOutFile == "") {
		sOutFile = m_sDefaultSavePath;
	}
	...
	std::vector<double> dMergedWaveform;
	std::vector<double> dCumWaveform;

	//std::ofstream outFileDSWF("../../downsampled_wfs.bin", std::ios::out | std::ios::binary);
	//std::ofstream outFileStartTime("../../start_time_2.bin", std::ios::out | std::ios::binary);
	//std::ofstream outFilePeaks("../../peaks_2.bin", std::ios::out | std::ios::binary);

	//outFileStartTime.write((char*)&iWindowStarts[0], sizeof(int64_t)* iNumPeaks);
	...
	for (int w = 0; w < iWindowStarts.size(); w++) {
			...
			while (iCurrentStart < iWindowEnds[w]) {
				if (bInWindow) {
					...
					for (int s = 0; s < HitsChannels[ch].waveforms[iChannelHitIndex[ch]].size(); s++) {
						dMergedWaveform[iSampleShift + s] += (double) HitsChannels[ch].waveforms[iChannelHitIndex[ch]][s];
					}
					iChannelCount++;
				}
		...
		dCumSum = 0.;
		dCumWaveform.resize(iWindowSize, 0.);
		for (int samp = 0; samp < iWindowSize; samp++) {
			if (dMergedWaveform[samp] > 0) {
		...
		m_d5pTime = PercentileTime(dCumWaveform, 0.05);
		m_d10pTime = PercentileTime(dCumWaveform, 0.10);
		m_d25pTime = PercentileTime(dCumWaveform, 0.25);
		m_d50pTime = PercentileTime(dCumWaveform, 0.50);
		m_d75pTime = PercentileTime(dCumWaveform, 0.75);
		m_d95pTime = PercentileTime(dCumWaveform, 0.95);
		...
		m_dCenterTime = iWindowStarts[w] + m_d50pTime;

		//Compute the height based times
		m_d10hTime = HeightTime(dMergedWaveform, 0.10);
		m_d90hTime = HeightTime(dMergedWaveform, 0.90);

		//Height based widths and times
		m_dRiseTimeHeight = m_d90hTime - m_d10hTime;
		m_dMaxTime = iWindowStarts[w] + arg_max(dMergedWaveform) * 4;

		Downsample(dMergedWaveform);
		...
		Peaks.combinedWaveforms.push_back(m_iDownsampledWaveform);

		dMergedWaveform.clear();
		m_iDownsampledWaveform.clear();
		iCoincidence = 0;
	}

	if (bSave){
		std::ofstream outFilePeaks(sOutFile, std::ios::out | std::ios::binary);
		...
		outFilePeaks.write((char*)&Peaks.areas[0], sizeof(double) * iNumPeaks);
		outFilePeaks.write((char*)&Peaks.centerTimes[0], sizeof(double) * iNumPeaks);
		outFilePeaks.write((char*)&Peaks.maxTimes[0], sizeof(double) * iNumPeaks);
		outFilePeaks.write((char*)&Peaks.range50pArea[0], sizeof(double) * iNumPeaks);
		outFilePeaks.write((char*)&Peaks.range90pArea[0], sizeof(double) * iNumPeaks);
		outFilePeaks.write((char*)&Peaks.riseTimeHeight[0], sizeof(double) * iNumPeaks);
		outFilePeaks.write((char*)&Peaks.riseTimeArea[0], sizeof(double) * iNumPeaks);

		for (int p = 0; p < iNumPeaks; p++) {
			outFilePeaks.write((char*)&Peaks.combinedWaveforms[p][0], sizeof(double) * m_pConfig->m_iMaxSamples);
		}
